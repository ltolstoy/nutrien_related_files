#At 5.29.19 - changed echelon_json schema to echelon_in (by Leo Tolstoy)
#Added writing_report function, to save reports (about missed files) 
# in s3:/emr-step-scripts-aws-account-silver/echelon_data_processing/reports
#6/3/19 - added more info to the report (whole processing time), changed file info collection to db!
...
def get_content_boto3(iterator):
	...
	result = []
	for x in iterator:
		if DEBUG: log('DEBUG: getting contents of file {}'.format(x))
		x_split = x.split('/')
		bucket = x_split[2]  #e.g. 'data-lake-us-east-2-549323063936-encrypted'
		path = '/'.join(x_split[3:]) #e.g. 'Agrian/AdHoc/NoETL/list/8.json'
		...
		while attempts < max_attempts:
			try:
				...
				if DEBUG: log('DEBUG: retrieved the body of the file')
				result.append(content) #Note: result is a list of contents of json files, distributed to current worker
				if DEBUG: log('DEBUG: appended body to results list')
				break
			except Exception as e:
				attempts += 1
				if attempts == max_attempts - 1: log("ERROR: get_content_boto3 got error with {}:\n{}".format(x, e))
...
def get_db_connection( db_user, db_host, db_port, db_name, ssm_pw_key):
...
def recreate_postgres_table(folder, engine, connection, cursor):
	...
	table_name = folder
	table_n = 'echelon_in.'+table_name # this form is for queries , otherwise doesn't work. 'echelon_in.field'
	if not engine.dialect.has_table(engine, table_name, schema='echelon_in'):  # If table don't exist, create it
...
def write_to_postgres(all_files, folder, cursor, connection):
	...
	rows_number = len(all_files) #how many rows we  are planning to write to db
	table_name = folder
	table_n = 'echelon_in.'+table_name # this form is for queries , otherwise doesn't work. 'echelon_in.field'
	...
	for raw_j in all_files:
		...
		if DEBUG: log('DEBUG: processing string {}'.format(j))
		if j != u'""' : # only process not-empty strings
			try:
				tmp_dict = {"id": json.loads(j)[folder]['id'],
					"json": j,
					"appended_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S") }# dict is a representation of record in postgres
				data.append(tmp_dict)
				if DEBUG: log('DEBUG: Successfully added tmp_dict to dictionaries list')
			except ValueError:
				log('ERROR: ValueError building json; source string was {}'.format(j))

		else:
			if DEBUG: log('DEBUG: empty string in all_files')
			empty_count += 1
	...
	log("\tWrote to db {} rows, while all_files has {} rows, found empty {} rows".format(cursor.rowcount, rows_number, empty_count))
	log('Exiting write_to_postgres')

def concatenation_parallel(ps3, hive_table_creation_files_location, folder, size_lim, db_user, db_host, db_port, db_name, ssm_pw_key):
	...
	report_message =[] #put new strings here, it will be written to s3 'reports' folder after all
	...
	s0 = time.time()
	engine, connection, cursor = get_db_connection( db_user, db_host, db_port, db_name, ssm_pw_key)
	...
	if s1-s0>60:
		rep_str = logging_prefix()+': '+'file info collection took '+ str(round((s1-s0)/60,1))+ 'min'
	else:
		rep_str = logging_prefix()+': '+'file info collection took '+ str(round(s1-s0,1))+ 'sec'
	report_message.append(rep_str)
	total_size=sum(size_list)
	log('{}: Found total files {}, with total size {} bytes'.format(folder, len(file_list), total_size) )
	rep_str = logging_prefix()+': '+'{}: Found total files {}, with total size {} bytes'.format(folder, len(file_list), total_size)
	report_message.append(rep_str)
	recreate_postgres_table(folder, engine, connection, cursor) # clean table before chunks or single data will be written there
	...
	if total_size > size_lim:
		...
		for i, ch in enumerate(chunks): #ch is an array of file names ['0.json','1,json']
			...
			s3=time.time()
			log_processing_time("\tparallelize files from full_path_chunk took", s2, s3)
			log("{}: Starting map part".format(folder))

			parts=rdd_f.mapPartitions(get_content_boto3).collect()
			log("{}: Done map part".format(folder))
			s4=time.time()
			log_processing_time("\t{}: parts collection took".format(folder), s3, s4)
			...
			try:
				...
				message = 'Successfully deleted then recreated Hive table for {}'.format(fpath)
			except Exception as e:
				message = 'ERROR: Failed to delete or create Hive table for {}'.format(fpath)
			...
			try:
				log('Invoking write_to_postgres from \'if total_size>{}, create chunks\' block'.format(size_lim))
				result = write_to_postgres(all_files_chunk, folder, cursor, connection)
				log('Successful invocation of write_to_postgres')
				message = 'Successfully wrote to postgres table related to {}'.format(folder)
			except Exception as epg:
				message = 'ERROR: Failure writing to postgres table related to {}'.format(folder)
				log(message)
			...
			log_processing_time("\t"+folder+": postgres writing chunk " + str(i+1) + " data took", s5, s6)
			log_processing_time(folder+": total time for chunk is ", s2, s6)
			log('{0} {2}  ETE is {1:.1f} min\n'.format(folder, (chunks_num - i-1)*(s5-s2)/60, str(datetime.now()).split('.')[0] ) )
			rep_str = logging_prefix()+': '+'total time for chunk is '+ str(round(s6-s2,1))+ 'sec'
			report_message.append(rep_str)
			message = 'Folder: {} : done writing to postgres for chunk {}'.format(folder, i+1)
		...
		if len(all_files) != len(file_list): #check if all files were read, or some are missing
			log('Collected {} of {} files'.format(len(all_files), len(file_list)))
			rep_str = logging_prefix()+': '+'Collected {} of {} files'.format(len(all_files), len(file_list))
			report_message.append(rep_str)
		s3=time.time()
		...
		try:
			...
			message = 'ERROR: Error Creating hive table data at {}/{}: {}'.format(bucket_name, fpath, e)
			log(message)
		s4=time.time()
		...
		try:
			...
			message = 'ERROR: Exception in write_to_postgres for folder {} invocation: {}'.format(folder, epg)
			log(message)
			log('ERROR: WHAT SHOULD WE DO HERE??')

		s5=time.time()
		log_processing_time("\t\t"+folder+": postgres writing took ", s4, s5)
		rep_str = logging_prefix()+': '+'postgres writing took'+ str(round(s5-s4,1))+ 'sec'
		report_message.append(rep_str)
		rep_str = logging_prefix()+': '+'total time for '+folder+' is '+ str(round(s5-s1,1))+ 'sec'
		report_message.append(rep_str)
	log('Closing connection to postgres')
	s_final = time.time()
	if s_final-s0>60:
		rep_str = logging_prefix()+': '+'whole processing took '+ str(round((s_final-s0)/60,1))+ 'min'
	else:
		rep_str = logging_prefix()+': '+'whole processing took '+ str(round(s_final-s0,1))+ 'sec'
	report_message.append(rep_str)
...
def main():
	...
	for folder in folders2process:
		try:
			...
			status = concatenation_parallel(ps3, hive_table_creation_files_location, folder, size_lim, db_user, db_host, db_port, db_name, ssm_pw_key) #BAD or OK
